...
Decade <- df$Decade
Average.Acousticness <- df$Acousticness

# Vectors
plot(Decade, Average.Acousticness, type = "l", color=average.values.df$Target) +
  title("Loudness vs Decade")
...
#aes(x=Decade, y=Average.Acousticness, color=df$Target)) +
  geom_line()
...
ggplot(filter(titanic, is.na(Survived)==FALSE), aes(Decade, fill=Survived)) + 
